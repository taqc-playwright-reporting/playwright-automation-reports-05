...
test.describe("GreenCityHome Page Guest Role", () => {
  ...
  test.describe("Header", () => {
    ...
    test("3. should display correct logo", async ({ page }) => {
      await expect(page.getByAltText(/green city logo/i)).toBeVisible();
    });
    test("4. should display navigation menu", async ({ page }) => {
      ...
      await expect(
        navigationMenu.getByRole("link", { name: "Eco news" }),
      ).toBeVisible();
      await expect(
        navigationMenu.getByRole("link", { name: "Events" }),
      ).toBeVisible();
      await expect(
        navigationMenu.getByRole("link", { name: "Places" }),
      ).toBeVisible();
      await expect(
        navigationMenu.getByRole("link", { name: "About us" }),
      ).toBeVisible();
      await expect(
        navigationMenu.getByRole("link", { name: "My space" }),
      ).toBeVisible();
      await expect(
        navigationMenu.getByRole("link", { name: "UBS courier" }),
      ).toBeVisible();
    });
    ...
    test("7. should display login button", async ({ page }) => {
      await expect(
        page.getByRole("img", { name: "sing in button" }),
      ).toBeVisible();
    });
    ...
    test("10. should home page has search input", async ({ page }) => {
      ...
      await page.getByRole("search", { name: "site search" }).click();
  ...
  test.describe("Main Content Section", () => {
    test("1. should display home page text", async ({ page }) => {
      await expect(
        page.getByRole("heading", {
          name: /A new way to cultivate useful habits/i,
        }),
      ).toBeVisible();
    });
    test('2. Hero CTA button "Start forming a habit!" is visible', async ({
      page,
    }) => {
      const button = page
        .getByRole("button", { name: /start forming a habit/i })
        .first();
    ...
    test("4. Newsletter subscription form accepts email input and Subscribe button is clickable", async ({
      page,
    }) => {
  ...
  test.describe("Footer Section", () => {
    test("1. Footer navigation links are visible", async ({ page }) => {
      ...
      const mainNavMenu = footer
        .getByRole("menu")
        .filter({ has: page.getByRole("link", { name: /Eco news/i }) });
      await expect
        .soft(mainNavMenu.getByRole("link"))
        .toHaveText([
          /Eco news/i,
          /Events/i,
          /Places/i,
    ...
    test('2. Footer "Follow us" social links are visible', async ({ page }) => {
      ...
      await expect(
        footer.getByRole("link", { name: /twitter/i }),
      ).toBeVisible();
      await expect(
        footer.getByRole("link", { name: /linkedin/i }),
      ).toBeVisible();
      await expect(
        footer.getByRole("link", { name: /facebook/i }),
      ).toBeVisible();
      await expect(
